...
def install_clang():
  ...
  try:
    ...
    if not os.path.exists("/usr/bin/clang"):
      base.cmd("ln", ["-s", "/usr/bin/clang-12", "/usr/bin/clang"])
    if not os.path.exists("/usr/bin/clang-cpp"):
      base.cmd("ln", ["-s", "/usr/bin/clang-cpp-12", "/usr/bin/clang-cpp"])
    if not os.path.exists("/usr/bin/clang++"):
      base.cmd("ln", ["-s", "/usr/bin/clang++-12", "/usr/bin/clang++"])
    if not os.path.exists("/usr/bin/dsymutil"):
      base.cmd("ln", ["-s", "/usr/bin/dsymutil-12", "/usr/bin/dsymutil"])
    if not os.path.exists("/usr/bin/llc"):
      base.cmd("ln", ["-s", "/usr/bin/llc-12", "/usr/bin/llc"])
    if not os.path.exists("/usr/bin/lli"):
      base.cmd("ln", ["-s", "/usr/bin/lli-12", "/usr/bin/lli"])
    if not os.path.exists("/usr/bin/lli-child-target"):
      base.cmd("ln", ["-s", "/usr/bin/lli-child-target-12", "/usr/bin/lli-child-target"])
    if(not os.path.exists("/usr/bin/llvm-PerfectShuffle")):
      base.cmd("ln", ["-s", "/usr/bin/llvm-PerfectShuffle-12", "/usr/bin/llvm-PerfectShuffle"])
    if not os.path.exists("/usr/bin/llvm-addr2line"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-addr2line-12", "/usr/bin/llvm-addr2line"])
    if not os.path.exists("/usr/bin/llvm-ar"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-ar-12", "/usr/bin/llvm-ar"])
    if not os.path.exists("/usr/bin/llvm-as"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-as-12", "/usr/bin/llvm-as"])
    if not os.path.exists("/usr/bin/llvm-bcanalyzer"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-bcanalyzer-12", "/usr/bin/llvm-bcanalyzer"])
    if not os.path.exists("/usr/bin/llvm-c-test"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-c-test-12", "/usr/bin/llvm-c-test"])
    if not os.path.exists("/usr/bin/llvm-cat"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-cat-12", "/usr/bin/llvm-cat"])
    if not os.path.exists("/usr/bin/llvm-cfi-verify"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-cfi-verify-12", "/usr/bin/llvm-cfi-verify"])
    if not os.path.exists("/usr/bin/llvm-config"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-config-12", "/usr/bin/llvm-config"])
    if not os.path.exists("/usr/bin/llvm-cov"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-cov-12", "/usr/bin/llvm-cov"])
    if not os.path.exists("/usr/bin/llvm-cvtres"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-cvtres-12", "/usr/bin/llvm-cvtres"])
    if not os.path.exists("/usr/bin/llvm-cxxdump"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-cxxdump-12", "/usr/bin/llvm-cxxdump"])
    if not os.path.exists("/usr/bin/llvm-cxxfilt"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-cxxfilt-12", "/usr/bin/llvm-cxxfilt"])
    if not os.path.exists("/usr/bin/llvm-cxxmap"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-cxxmap-12", "/usr/bin/llvm-cxxmap"])
    if not os.path.exists("/usr/bin/llvm-diff"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-diff-12", "/usr/bin/llvm-diff"])
    if not os.path.exists("/usr/bin/llvm-dis"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-dis-12", "/usr/bin/llvm-dis"])
    if not os.path.exists("/usr/bin/llvm-dlltool"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-dlltool-12", "/usr/bin/llvm-dlltool"])
    if not os.path.exists("/usr/bin/llvm-dwarfdump"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-dwarfdump-12", "/usr/bin/llvm-dwarfdump"])
    if not os.path.exists("/usr/bin/llvm-dwp"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-dwp-12", "/usr/bin/llvm-dwp"])
    if not os.path.exists("/usr/bin/llvm-elfabi"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-elfabi-12", "/usr/bin/llvm-elfabi"])
    if not os.path.exists("/usr/bin/llvm-exegesis"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-exegesis-12", "/usr/bin/llvm-exegesis"])
    if not os.path.exists("/usr/bin/llvm-extract"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-extract-12", "/usr/bin/llvm-extract"])
    if not os.path.exists("/usr/bin/llvm-ifs"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-ifs-12", "/usr/bin/llvm-ifs"])
    if not os.path.exists("/usr/bin/llvm-install-name-tool"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-install-name-tool-12", "/usr/bin/llvm-install-name-tool"])
    if not os.path.exists("/usr/bin/llvm-jitlink"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-jitlink-12", "/usr/bin/llvm-jitlink"])
    if not os.path.exists("/usr/bin/llvm-lib"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-lib-12", "/usr/bin/llvm-lib"])
    if not os.path.exists("/usr/bin/llvm-link"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-link-12", "/usr/bin/llvm-link"])
    if not os.path.exists("/usr/bin/llvm-lipo"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-lipo-12", "/usr/bin/llvm-lipo"])
    if not os.path.exists("/usr/bin/llvm-lto"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-lto-12", "/usr/bin/llvm-lto"])
    if not os.path.exists("/usr/bin/llvm-lto2"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-lto2-12", "/usr/bin/llvm-lto2"])
    if not os.path.exists("/usr/bin/llvm-mc"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-mc-12", "/usr/bin/llvm-mc"])
    if not os.path.exists("/usr/bin/llvm-mca"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-mca-12", "/usr/bin/llvm-mca"])
    if not os.path.exists("/usr/bin/llvm-modextract"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-modextract-12", "/usr/bin/llvm-modextract"])
    if not os.path.exists("/usr/bin/llvm-mt"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-mt-12", "/usr/bin/llvm-mt"])
    if not os.path.exists("/usr/bin/llvm-nm"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-nm-12", "/usr/bin/llvm-nm"])
    if not os.path.exists("/usr/bin/llvm-objcopy"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-objcopy-12", "/usr/bin/llvm-objcopy"])
    if not os.path.exists("/usr/bin/llvm-objdump"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-objdump-12", "/usr/bin/llvm-objdump"])
    if not os.path.exists("/usr/bin/llvm-opt-report"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-opt-report-12", "/usr/bin/llvm-opt-report"])
    if not os.path.exists("/usr/bin/llvm-pdbutil"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-pdbutil-12", "/usr/bin/llvm-pdbutil"])
    if not os.path.exists("/usr/bin/llvm-profdata"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-profdata-12", "/usr/bin/llvm-profdata"])
    if not os.path.exists("/usr/bin/llvm-ranlib"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-ranlib-12", "/usr/bin/llvm-ranlib"])
    if not os.path.exists("/usr/bin/llvm-rc"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-rc-12", "/usr/bin/llvm-rc"])
    if not os.path.exists("/usr/bin/llvm-readelf"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-readelf-12", "/usr/bin/llvm-readelf"])
    if not os.path.exists("/usr/bin/llvm-readobj"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-readobj-12", "/usr/bin/llvm-readobj"])
    if not os.path.exists("/usr/bin/llvm-reduce"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-reduce-12", "/usr/bin/llvm-reduce"])
    if not os.path.exists("/usr/bin/llvm-rtdyld"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-rtdyld-12", "/usr/bin/llvm-rtdyld"])
    if not os.path.exists("/usr/bin/llvm-size"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-size-12", "/usr/bin/llvm-size"])
    if not os.path.exists("/usr/bin/llvm-split"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-split-12", "/usr/bin/llvm-split"])
    if not os.path.exists("/usr/bin/llvm-stress"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-stress-12", "/usr/bin/llvm-stress"])
    if not os.path.exists("/usr/bin/llvm-strings"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-strings-12", "/usr/bin/llvm-strings"])
    if not os.path.exists("/usr/bin/llvm-strip"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-strip-12", "/usr/bin/llvm-strip"])
    if not os.path.exists("/usr/bin/llvm-symbolizer"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-symbolizer-12", "/usr/bin/llvm-symbolizer"])
    if not os.path.exists("/usr/bin/llvm-tblgen"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-tblgen-12", "/usr/bin/llvm-tblgen"])
    if not os.path.exists("/usr/bin/llvm-undname"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-undname-12", "/usr/bin/llvm-undname"])
    if not os.path.exists("/usr/bin/llvm-xray"):
      base.cmd("ln", ["-s", "/usr/bin/llvm-xray-12", "/usr/bin/llvm-xray"])
    if not os.path.exists("/usr/bin/not"):
      base.cmd("ln", ["-s", "/usr/bin/not-12", "/usr/bin/not"])
    if not os.path.exists("/usr/bin/obj2yaml"):
      base.cmd("ln", ["-s", "/usr/bin/obj2yaml-12", "/usr/bin/obj2yaml"])
    if not os.path.exists("/usr/bin/opt"):
      base.cmd("ln", ["-s", "/usr/bin/opt-12", "/usr/bin/opt"])
    if not os.path.exists("/usr/bin/verify-uselistorder"):
      base.cmd("ln", ["-s", "/usr/bin/verify-uselistorder-10", "/usr/bin/verify-uselistorder"])
    if not os.path.exists("/usr/bin/sanstats"):
      base.cmd("ln", ["-s", "/usr/bin/sanstats-12", "/usr/bin/sanstats"])
    if not os.path.exists("/usr/bin/yaml-bench"):
      base.cmd("ln", ["-s", "/usr/bin/yaml-bench-12", "/usr/bin/yaml-bench"])
    if not os.path.exists("/usr/bin/yaml2obj"):
      base.cmd("ln", ["-s", "/usr/bin/yaml2obj-12", "/usr/bin/yaml2obj"])
    if not os.path.exists("/usr/bin/ld.lld"):
      base.cmd("ln", ["-s", "/usr/bin/ld.lld-12", "/usr/bin/ld.lld"])
    if not os.path.exists("/usr/bin/lld"):
      base.cmd("ln", ["-s", "/usr/bin/lld-12", "/usr/bin/lld"])
    if not os.path.exists("/usr/bin/ld64.lld"):
      base.cmd("ln", ["-s", "/usr/bin/ld64.lld-12", "/usr/bin/ld64.lld"])
    if not os.path.exists("/usr/bin/lld-link"):
      base.cmd("ln", ["-s", "/usr/bin/lld-link-12", "/usr/bin/lld-link"])
